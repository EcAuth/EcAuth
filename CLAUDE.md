...
| **値の変換**（ハッシュ化・形式変更・既存カラムの書き換え） | **リリースを 2 つに分割**し、各リリースは `migrate-and-deploy` | 旧コードが変換後 DB に旧形式を書き込む窓を塞ぐ |
| **削除・制約強化**（カラム / テーブル削除、リネーム、UNIQUE 追加） | **リリースを 2 つに分割**し、各リリースは `migrate-and-deploy` | 旧コードが存在しない列を参照して落ちる。制約強化は旧コードが違反データを書き得る |

#### リリースの分割

互換コードの merge と マイグレーションの merge を分ける。**同じ PR に入れない。**

| リリース | 内容 | migrate 実行時に動いているコード |
|---------|------|------------------------------|
| 1（expand） | 旧構造を参照しなくなったコード（読みは新旧両対応、書きは新形式）。マイグレーションは追加のみ、または無し | 旧コード。migrate は無害または no-op |
| 2（contract） | 値の変換 / 削除 / 制約強化のマイグレーション。コード変更は原則入れない | リリース 1 の互換コード。両形式を扱える |

分割すればリリース 2 の migrate 時点で互換コードが既に稼働しているため、**staging の自動実行でも
production の `migrate-and-deploy` でも安全**になる。EcAuthDocs#110 が実際にこの形（EcAuth#521 が
expand、`b2b_user.external_id` の削除が contract）。
...
（EcAuth#521 のレビュー指摘 4 件のうち 2 件は二重書きに起因していた）。書きを新形式のみにしておくことが、
リリース 2 の migrate 中に旧コードが不整合を書かない根拠にもなる。
