...
| **値の変換**（ハッシュ化・形式変更・既存カラムの書き換え） | **リリースを分割**し、各リリースは `migrate-and-deploy` | 旧コードが変換後 DB に旧形式を書き込む窓を塞ぐ |
| **削除・リネーム**（カラム / テーブル削除、リネーム） | **リリースを 3 段に分割**し、各リリースは `migrate-and-deploy` | 旧コードが存在しない列を参照して落ちる。フォールバック実装も旧構造を参照するため除去リリースが要る |
| **制約強化**（UNIQUE 追加） | **リリースを分割** + 事前の重複解消（後述） | 旧コードが違反データを書き得る。既存データの重複は順序では解決しない |

#### リリースの分割

互換コードの merge とマイグレーションの merge を分ける。**同じ PR に入れない。**
削除・リネームでは**さらに旧構造への参照を除去するリリース**が要るので、全部で 3 段になる。

| リリース | コード | マイグレーション | migrate 実行時に動いているコード |
|---------|-------|----------------|------------------------------|
| 1（expand） | 新構造へ**二重書き** + 読みは新旧両対応 | 新構造の追加 + backfill | 旧コード。追加のみなので無害 |
| 2（transition） | 旧構造への参照を**完全に除去**（EF マッピングを含む） | 無し | リリース 1 の互換コード |
| 3（contract） | 変更なし | 旧構造の削除 / 値の変換 / 制約強化 | リリース 2 のコード。旧構造を一切参照しない |
