...
{
    /// <summary>
    /// b2b_user_identity.external_id（メールアドレスや旧プラグインの login_id 等、個人情報を
    /// 含み得る値）を永続化・検索する際に適用するハッシュ化ヘルパー。
    ///
    ...
    /// account_owner（external_id = メールアドレス）と一般管理者（external_id = EC-CUBE の
    /// member_id。EcAuthDocs#110 以前のプラグインは login_id）の両経路で一貫したハッシュ値を得る。
    ///
    /// 正規化に ToLowerInvariant を含むため、文字列の external_id は実質的に大文字小文字を
    /// 区別しない（同一発行元で大小のみ異なる login_id は同一ユーザーへ解決される）。
    /// </summary>
